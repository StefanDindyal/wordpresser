...
public function __construct() {
    ...
    if (isset($_GET['page']) && ( $_GET['page'] == 'bb-settings') ){
		add_action( 'admin_enqueue_scripts', array( $this, '_bbAdminScripts' ) );
		add_action( 'admin_head', array( $this, '_bbaddCustomScripts' ) );
	}

}
	
public function _bbdisplaySettings( $field, $meta = null, $repeatable = null ) {
   ...
   switch($type) {
      ...
      case 'checkbox':
         ...
         echo '<input type="checkbox" value="'.($meta != '' ? 'true' : 'false').'" name="'.$name.'" id="'.$id.'" ',$meta != '' ? ' checked="checked"' : '',' />
         	<label for="bb_options['.$field['id'].']"><span class="description">'.$field['desc'].'</span></label>';
         echo '</div>';
      break;
      // value checkbox
		case 'value_checkbox':
      ...
      case 'radio':
         foreach ( $field['options'] as $option ) {
         	echo '<input type="radio" name="'.$name.'" id="bb_options['.$option['value'].']" value="'.$option['value'].'" ',$meta == $option['value'] ? ' checked="checked"' : '',' />
      ...
      case 'checkbox_group':
		 	foreach ($field['options'] as $option){
		 	    echo '<input type="checkbox" value="'.$option['value'].'" name="bb_options['.$field['id'].'][]" id="bb_options['.$option['value'].']"',$meta && in_array($option['value'], $meta) ? ' checked="checked"' : '',' />
		 	    		<label for="'.$option['value'].'">'.$option['label'].'</label><br />';
		 	}
         echo $desc;
      break;
      ...
      // image
	  case 'image':
	      $image = BB_URL . '/images/fb-image.jpg';	
	      $image = '';
	      echo '<div class="rg-bb_image" style="overflow:hidden;padding:0 0 0 10px">
	      		<span class="rg-bb_default_image" style="display:none">' . $image . '</span>';
	      if ( $meta ) { $image = wp_get_attachment_image_src( intval( $meta ), 'medium' ); }				
	      echo	'<input name="'.$name.'" type="hidden" class="rg-bb_upload_image" value="' . intval( $meta ) . '" style="width:'.$width.';" />
	      	     <img src="'.($meta ? $image[0] : $image).'" class="rg-bb_preview_image" style="width:150px;" alt="( Insert Image )" /><br>
	      		 <a href="#" class="rg-bb_upload_image_button button" style="margin: 10px 0;" rel="' . get_the_ID() . '">Choose Image</a>
	      		 <small style="display:inline-block;margin-top:15px;margin-left:10px;"><a href="#" class="rg-bb_clear_image_button">Remove Image</a></small><br>'.$desc.'
	      	   </div>';
	  break;
      // repeatable
      case 'repeatable':
		    ...
		    foreach( $meta as $value ):
		    	...
		    	echo '<li class="r-row">';
					...
					foreach($repeatable_fields as $repeatable_field):
					    ...
					    echo $this->_bbdisplaySettings( $repeatable_field, $meta[$i][$p++], array( $id, $i ) );
					endforeach;
			    	echo '<div style="display:inline-block; margin-left: 10px; vertical-align:baseline;" class="repeatable-remove button" href="#">Remove field</div>';
			    echo '</li>';
			    $i++;
		    endforeach;
		    echo '</ul>';
		  ...
		  foreach( $meta as $value ):
		  	  ...
		  	  foreach($group_fields as $group_field ):
		  	    echo '<li class="r-row">';
		  	      ...
		  	      echo $this->_bbdisplaySettings( $group_field, $meta[$i][$p++], array( $id, $i ) );
		  	    echo '</li>';
		  	  endforeach;
		  	  $i++;
		  endforeach;
...
function _bbfindFieldType( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return $this->_bbfindFieldType( $needle, $h['repeatable_fields'] );
		elseif ( isset( $h['type'] ) && $h['type'] == 'input_group' )
			return $this->_bbfindFieldType( $needle, $h['group_fields'] );
		elseif ( ( isset( $h['type'] ) && $h['type'] == $needle ) || ( isset( $h['repeatable_type'] ) && $h['repeatable_type'] == $needle ) )
			return true;
	return false;
}

function _bbfindRepeatableType( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
...
function _bbvalidateThis($input) {
    $valid = array();
...
function _bbdisplayPage() {
   ...
   echo '<div class="wrap" id="rg-bb">';
      	...
      	if ( ! isset( $_REQUEST['settings-updated'] ) )
   		...
   		if( $_GET['page'] == 'bb-settings' && isset($_GET['all-transients-deleted']) ):
		    echo '<div class="updated fade below-h2"><p>All Transients Deleted</p></div>';
		elseif( $_GET['page'] == 'bb-settings' && isset($_GET['transient-deleted']) ):
      ...
      echo '<div class="has-right-sidebar">';
      	...
      	echo '<div style="float: left; width: 75%;" >';
      		echo '<form method="post" action="options.php" class="bb_options_form">';
      			settings_fields('bb_options');
      			wp_nonce_field( basename(__FILE__), 'bb_Admin_Page_Class_nonce' );
      			
      			echo '<div id="section_container">';
      			echo '<table class="form-table rg-bb">';
						foreach ( $bb_fields as $field ) {
						   if ( $field['type'] == 'section' ) {
						   	...
						   	echo '<tr class="' . $field['id'] . '">
						   			...
						   			$meta = get_option( 'bb_options' );
						   			echo $this->_bbdisplaySettings( $field, $meta[$field['id']] );
...
public function _bbregisterSettings() {
	register_setting( 'bb_options', 'bb_options', array( $this, '_bbvalidateThis' ) );
}
...
public function _categoryOrderGetSubCats($parentID){
  ...
  $subCatStr = "";
  $results=$wpdb->get_results("SELECT t.term_id, t.name FROM $wpdb->term_taxonomy tt, $wpdb->terms t, $wpdb->term_taxonomy tt2 WHERE tt.parent = $parentID AND tt.taxonomy = '".bbTAXONOMY."' AND t.term_id = tt.term_id AND tt2.parent = tt.term_id GROUP BY t.term_id, t.name HAVING COUNT(*) > 0 ORDER BY t.term_order ASC");
...
public function _categoryOrderCatQuery($parentID){
  global $wpdb;
  return $wpdb->get_results("SELECT * FROM $wpdb->terms t inner join $wpdb->term_taxonomy tt on t.term_id = tt.term_id WHERE taxonomy = '".bbTAXONOMY."' and parent = $parentID ORDER BY term_order ASC");
}
...
----------------------------------------- */
public function _bbAdminScripts() {
	global $bb_fields;
	if( is_admin() && isset($_GET['page']) && ( $_GET['page'] == 'bb-settings' ) ) {
	
		$deps = array( 'jquery' );
		wp_enqueue_media();
		
		if ( $this->_bbfindFieldType( 'spinnerslider', $bb_fields ) || $this->_bbfindFieldType( 'slider', $bb_fields ) )
			wp_enqueue_script('jquery-ui-slider');
			wp_enqueue_script('spinner', BB_URL . '/js/spin.js', $deps, true);
			
		if ( $this->_bbfindFieldType( 'repeatable', $bb_fields ) || $this->_bbfindFieldType( 'input_group', $bb_fields ) )
			wp_enqueue_script('jquery-ui-sortable');
		
		wp_enqueue_script('thickbox', null, $deps );
		wp_enqueue_script('wp-color-picker');
		wp_enqueue_script('jquery-ui-datepicker');
		wp_enqueue_script('chosen', BB_URL . '/js/chosen.js', $deps, true );
		wp_enqueue_script('admin-js', BB_URL . '/js/custom-admin.js', $deps, true);
		
		wp_enqueue_style('thickbox');
		wp_enqueue_style('wp-color-picker');
		wp_enqueue_style('chosen-css', BB_URL. '/css/chosen.css' );
		wp_enqueue_style('admin-css', BB_URL . '/css/admin-panel.css');
		wp_enqueue_style('jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.8/themes/base/jquery-ui.css');		
	}
}  	

public function _bbaddCustomScripts() {
   global $bb_fields;
