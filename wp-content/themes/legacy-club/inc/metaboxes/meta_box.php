...
if(is_admin()){
	wp_enqueue_script( 'jquery', '//ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js', array( 'jquery' ));
}
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'email':
		default:
			echo '<input type="' . $type . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" class="regular-text" style="' . esc_attr( $width ) . '" size="30" />'. $desc;
		break;
		case 'url':
		...
		// textarea
		case 'textarea':
			echo '<textarea name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" cols="60" rows="' . esc_attr( $rows ) . '" style="' . esc_attr( $width ) . '">' . esc_textarea( $meta ) . '</textarea>' . $desc;
		break;
		// editor
		case 'editor':
		...
		case 'color':
			$meta = $meta ? $meta : '#';
			echo '<input type="text" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '" size="10" class="regular-text" />' . $desc;
			echo '<div id="colorpicker-' . esc_attr( $id ) . '"></div>
				<script type="text/javascript">
				jQuery(function($) {
					$("#colorpicker-' . esc_attr( $id ) . '").hide();

					$("#colorpicker-' . esc_attr( $id ) . '").farbtastic("input#' . esc_attr( $id ) . '");
					
					$("#' . esc_attr( $id ) . '").live("blur", function() { $("#colorpicker-' . esc_attr( $id ) . '").slideUp(250); } );
					
					$("#' . esc_attr( $id ) . '").live("focus", function() { $("#colorpicker-' . esc_attr( $id ) . '").slideDown(250); } );
				});
				</script>';
		break;
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				echo '<div class="btns"><span class="sort hndle"></span><a class="meta_box_repeatable_remove" href="#"></a></div>';
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( in_array( true, array(
				...
				wp_register_script( 'chosen', get_template_directory_uri(). '/inc/metaboxes/js/chosen.js', array( 'jquery' ) );
				$deps[] = 'chosen';
			...
			if ( in_array( true, array( 
				...
				wp_enqueue_script( 'farb', get_template_directory_uri(). '/inc/metaboxes/js/farbtastic.js', $deps );
			...
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'farbtastic', get_template_directory_uri(). '/inc/metaboxes/css/farb.css', $deps );
...
/* RETURN AN IMAGE ID */
/* ------------------------------------------------------------------*/
function rg_legacy_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
	$attachment_id = false;
	...
	// Make sure the upload path base directory exists in the attachment URL, to verify that we're working with a rgnrtr library image
