...
class TFuse_Widget_Selected_Pages extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        $class = 'box box_menu widget_tfuse_posts';
    ...
    <?php
        ...
        <ul  class="third_menu">
            ...
            foreach ($pages as $key=>$val) {
                ...
                if ($key == @$post->ID) {$active = ' current-menu-item ';} else $active='';

                $page = get_post($key);
                echo '<li class="'.$first.$last.$active.'"><a href="' . get_page_link($key) . '">' . $page->post_title . '</a></li>';
                $page = get_post($key);
            } ?>
