...
jQuery(document).ready(function($) {
        ...
        if(extra!=undefined && extra.custom!=undefined && (extra.custom.slide_src!=undefined || extra.custom.slide_media!=undefined)) {
            ...
            if(src!='') {
                ...
                if(-1==$.inArray(src.split('.').pop().toLowerCase(),image_ext)) {
                    ...
                    $obj.find('img').attr('src',format_slide_src(src)).attr('preview',src);
            }
            else {
                $obj.find('img').hide();
            }
    ...
    function format_slide_src(src) {
        return tf_script.TFUSE_THEME_URL+'/framework/timthumb/timthumb.php?h=144&w=144&zc=1&src='+thumb_link(src);
    }
